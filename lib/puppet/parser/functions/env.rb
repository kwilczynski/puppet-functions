...
module Puppet::Parser::Functions
  newfunction(:env, :type => :rvalue, :doc => <<-EOS
Returns
...
For example:
  ...
  EOS
    ...
    if arguments.size < 1
      raise(Puppet::ParseError, "env(): Wrong number " +
    ...
    values = values.inject([]) do |array, v|
      ...
      if not [Array, String].include?(klass)
        raise(Puppet::ParseError, 'env(): Requires ' +
    ...
    values.each do |i|
      ...
      if not i.is_a?(String)
        raise(Puppet::ParseError, 'env(): Requires ' +
          'environment variable to be a string')
      end
      ...
      if i.empty?
        ...
        #
        # Get the value of interest from the environment ...
        #
